...
for (t in 1:(n.years-1)){
  ...
  N[t+1]<-N[t]*(exp(rV[t])/(1+(N[t]*gamma)^beta))*exp(-FR[t]*P[t]/N[t])
...
# Initial values
inits <- function () {
  list(sigma_V=runif(1,0.1,2), sigma_P=runif(1,0.1,2), r_V=runif(1,0.1,2),r_P=runif(1,0.1,2), gamma=runif(1,0.1,5), Q=runif(1,0,5),tau_FR=runif(1,1,10),C=runif(1,10,100),D=runif(1,0.01,30))}
...
pdf(file="GaussianFuncResp_fittedonVariableD.pdf",width=8,height=5)
par(mfrow=c(1,1),cex=1.2)
fr_fit<-nls(FR~CE*N/(DE+N),start=list(CE=1,DE=1))
CE<-coef(fr_fit)[1]
DE<-coef(fr_fit)[2]
plot(N,FR,ylim=c(0,3),lwd=2)
...
cat("
    ...
    logNupdate[t] <- logN[t] + r_V -log(1+N[t]*gamma) -C*exp(logP[t])/(D+N[t])
    logN[t+1] ~ dnorm(logNupdate[t],tau_V)
    N[t]<-exp(logN[t])
...
# Initial values
inits <- function () {
  list(sigma_V=runif(1,0.1,2), sigma_P=runif(1,0.1,2), r_V=runif(1,0.1,2),r_P=runif(1,0.1,2), gamma=runif(1,0.1,5), Q=runif(1,0,5),C=runif(1,10,100),D=runif(1,0.01,0.1))}
...
# Initial values
inits <- function () {
  list(sigma_V=runif(1,0.1,2), sigma_P=runif(1,0.1,2), r_V=runif(1,0.1,2),r_P=runif(1,0.1,2), gamma=runif(1,0.1,5), Q=runif(1,0,5),sigma_FR=runif(1,0.0000001,0.001),C=runif(1,10,100),mD=runif(1,0.5,3),sD=runif(1,0.5,6))}

# Parameters monitored
parameters<-c("r_V","gamma","r_P","Q","sigma2_V","sigma2_P","mD","sD","C","sigma_FR")#"logN","logP","FR"
