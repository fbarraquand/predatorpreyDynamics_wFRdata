...
### 08/02/2019 Adding a very small obs error on densities for identifiability purposes
...
### Observation error added (it's really a small, precision error - on the real scale)
obs_error = rnorm(n.years,0,0.1)
# defined here so that the rest of the simulation is identical (RNG seed set below)
...
plot(1:n.years,N,type="b")
lines(1:n.years,N+obs_error,type="b",col="red")

### Produce dataset to fit
data = cbind(log(N+obs_error),log(P),FR) 
