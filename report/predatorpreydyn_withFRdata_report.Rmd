...
In the simplest case highlighted by our discrete-time dynamical systems of the sections above, $p_{1}(x|y)=p_{11}(x_{1}|y)p_{12}(x_{2}|y)$ is the product of the two gaussian pdf for log-densities conditional on past densities and auxiliary information. Using the equations $\ref{eq:prey_discreteLeslieMay}$  and $\ref{eq:predator_discreteLeslieMay}$, we obtain the following difference equation on the log-scale:
