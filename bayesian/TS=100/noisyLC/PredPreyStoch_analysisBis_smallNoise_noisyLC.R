...
### FB 26/01/2019 Noisy limit cycle parameter values
...
### plot densities
library(mcmcplots)
png(file="Densities_withFRdata.png", width = 1200, height = 1200,res=300)
denplot(out,c("r_V","K_V","r_P","Q","sigma2_V","sigma2_P","C","D"))
dev.off()
png(file="Densities_withoutFRdata.png", width = 1200, height = 1200,res=300)
...
for (i in 1:n){
  ...
  if (i == 1){plot(Nprey,FRstoch[i,],type='l',lwd=0.5,col=alpha('black',0.05),ylab='Functional response',ylim=c(1,20),xlim=c(1,10),xlab='N prey',main='With (C,D) correlations')
  }
...
for (i in 1:n){
  ...
  if (i == 1){plot(Nprey,FRstoch[i,],type='l',lwd=0.5,col=alpha('black',0.05),ylab='Functional response',ylim=c(1,20),xlim=c(1,10),xlab='N prey',main='Without (C,D) correlations')
  }
  else {lines(Nprey,FRstoch[i,],type='l',lwd=0.5,col=alpha('black',0.05))}
}
