...
cat("
    ...
    logC ~ dnorm(0,1) #C~dgamma(.01,.01)
    logD ~ dnorm(0,1) #D~gamma(.01,.01)
    C<-exp(logC)
    D<-exp(logD)
...
# K_V          1.99    1.21   0.45   1.15   1.74   2.54   4.93    1  2200
# Q            9.93    3.42   4.53   7.44   9.57  11.91  17.63    1  3000
# logC        -0.78    0.78  -2.43  -1.28  -0.75  -0.25   0.70    1  3000
# logD         0.36    0.97  -1.57  -0.29   0.35   1.01   2.24    1  3000
...
C
D
exp(out$BUGSoutput$mean$logC)
exp(out$BUGSoutput$mean$logD)
#OK! In this case it works but changes in C produce different results