...
write.table(x=jags.sum,file="JAGSsummary_PredPreyStoch_RMA_QC_wKR.txt")
# MCMC Output
...
write.table(x=jags.sum,file="JAGSsummary_PredPreyStoch_RMA_QC_woutKR.txt")
...
png(file="PairPosteriorPlot_withFRdata_QC.png", width = 1200, height = 1200,res=300)
...
png(file="PairPosteriorPlot_withoutFRdata_QC.png", width = 1200, height = 1200, res=300)
pairs(postsamples2,c("r_V","K_V","r_P","epsilon","C","D"))
dev.off()

pdf(file="PairCorrelPosteriorPlot_QC.pdf",width = 4,height = 8)
par(mfrow=c(2,1))
...
### With both C and D
pdf(file="PPO_QC_aftertransients.pdf",width=12,height=8)
